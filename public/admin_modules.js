...
function loadEquipment() {
    fetch('/getEquipment')
        ...
        .then(data => {
            ...
            const selectedVehicle = document.getElementById('sortVehicleAssignment').value; // Marked change: Get selected vehicle
            const searchQuery = document.getElementById('inventorySearchBox').value.toLowerCase(); // Marked change: Get search query

            const filteredData = data.filter(item => {
                const matchesVehicle = selectedVehicle ? item.vehicleAssignment === selectedVehicle : true; // Marked change: Filter by vehicle
                const matchesSearch = item.itemName.toLowerCase().includes(searchQuery); // Marked change: Filter by search query
                return matchesVehicle && matchesSearch; // Marked change: Filter based on both vehicle and search query
            });
            ...
            filteredData.forEach(item => {
                ...
                div.innerHTML = `
                    ...
                    <p class="text-base font-Inter text-center">${item.itemName}</p>
                    <p class="text-sm font-Inter text-center">${item.vehicleAssignment}</p> <!-- Marked change: Added vehicle assignment -->
                `;
                container.appendChild(div);
            });
        })
