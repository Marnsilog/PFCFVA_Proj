...
function loadEquipment(search = '') {
    const vehicleAssignmentValue = document.getElementById('sortVehicleAssignment').value.trim() || '';
    const itemStatusValue = document.getElementById('itemStatus').value.trim() || '';
    
    const url = `/getEquipment?search=${encodeURIComponent(search)}&vehicleAssignment=${encodeURIComponent(vehicleAssignmentValue)}&itemStatus=${encodeURIComponent(itemStatusValue)}`;
    ...
    fetch(url)
        ...
        .then(data => {
            ...
            data.forEach((item, index) => {
                ...
                div.innerHTML = `
                    <div class="flex justify-normal space-x-5 py-2">
                        <p class="md:py-8 md:ml-5 ml-3 text-xl py-2 md:text-3xl">${index + 1}</p>
                        <img src="${item.itemImage}" class="object-fill w-10 h-10 md:w-24 md:h-24 rounded-md" alt="Equipment Image">
                        <div class="md:w-full w-full flex justify-start md:justify-between">
                            <p class="md:py-8 py-2 md:ml-5 text-sm md:text-2xl font-bold">${item.itemName}</p>
                            <p class="md:py-8 py-2 md:ml-5 text-sm md:text-xl font-[300] pl-6">${item.vehicleAssignment}</p>
                        </div>
                    </div>
            ...
            // Add the count div at the bottom
            const countDiv = document.createElement('div');
            countDiv.className = 'text-center mt-4 text-lg font-bold';
            countDiv.innerText = `Total Items: ${data.length}`; // Display the total count
            container.appendChild(countDiv); // Append the count div after all items
